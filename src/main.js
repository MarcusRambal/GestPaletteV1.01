...
ipcMain.handle('print-ticket', async (event, invoice) => {
  // 1. Creamos la ventana oculta para el ticket
  ...
  // Generamos el HTML (Tu lógica se mantiene igual)
  const productosHTML = invoice.productos.map(p => `
    <div class="item">
      <strong>${p.nombre}</strong><br>
      <strong>Tipo: ${p.tipo}</strong><br>
      ${p.cantidad} x $${p.precio.toLocaleString()}<br>
      <span>Subtotal: $${p.total.toLocaleString()}</span>
    </div>
  `).join('');
    ...
    <html>
      <head>
        <style>
          @page { margin: 0; }
          body { 
            width: 160px; /* Ajustado para dar margen de seguridad en papel de 58mm */
          ...
          .dashed { border-top: 1px dashed black; margin: 8px 0; width: 100%; }
          .total { font-weight: bold; font-size: 14px; margin: 10px 0; }
          .item { margin-bottom: 8px; line-height: 1.2; }
        </style>
      </head>
      <body>
        <div class="text-center">
          <strong style="font-size: 14px;">FRUPALETTE</strong><br>
          Factura de Venta<br>
        ...
        <div class="pago-info">
          Pago: ${invoice.tipoPago.toUpperCase()}<br>
          ${invoice.multipagos ? `<strong>Ref: Multipago activo</strong>` : ''}
        </div>
  ...
  // 2. Cargamos el contenido y esperamos a que esté listo
  await printWin.loadURL(`data:text/html;charset=utf-8,${encodeURI(htmlContent)}`);

  const printers = await printWin.webContents.getPrintersAsync();
  const printerExists = printers.find(p => p.name === 'ZJ-58');

  if (!printerExists) {
    console.error("La impresora ZJ-58 no está disponible. Detectadas:", printers.map(p => p.name));
    printWin.close();
    return { success: false, error: "Impresora no encontrada" };
  }

  // 4. EJECUTAR IMPRESIÓN
  return new Promise((resolve) => {
    printWin.webContents.print({
      ...
      if (success) {
        console.log('Impresión exitosa en ZJ-58');
        resolve({ success: true });
      } else {
        console.error('Fallo de impresión:', errorType);
