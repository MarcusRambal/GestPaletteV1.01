...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    async function printTicket (invoice) {
      try {
        const result = await window.paletteAPI.Printer.printTicket(invoice)
        if (result.success) {
          console.log('Impresión:', result.message)
        } else {
          // Mostrar error
          alert('✗ Error en la impresión: ' + result.message)
          console.error('Error de impresión:', result.message)
        }
        return result
      }
      catch (error) {
        console.error('Error al imprimir el ticket:', error)
        alert('✗ Error inesperado en la impresión: ' + error.message)
